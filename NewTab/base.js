...
function search_engine_options_html() {
	if (EXTENSION_STORE_MODE) {
		return '<option value="' + BROWSER_DEFAULT_SEARCH_ENGINE_ID + '">' + SEARCH_ENGINES.browserDefault.label + '</option>'
	}
...
function canUseChromeSearchApi() {
	return Boolean(window.chrome?.search?.query || hasChromeExtensionRuntime() || window.self !== window.top)
}

function requestBrowserDefaultSearch(query) {
	...
	if (hasChromeExtensionRuntime()) {
		return new Promise(resolve => {
			chrome.runtime.sendMessage({
				source: 'DYG_NEW_TAB_PAGE',
				type: 'SEARCH_DEFAULT_PROVIDER',
				query
			}, response => {
	...
	if (!canUseGoogleExtensionAuth()) return Promise.resolve(false)

	const requestId = ++googleExtensionAuthRequestId
	return new Promise(resolve => {
		const timeoutId = setTimeout(() => {
			delete googleExtensionAuthRequests[requestId]
			resolve(false)
		}, EXTENSION_AUTH_REQUEST_TIMEOUT)

		googleExtensionAuthRequests[requestId] = {
			resolve: () => {
...
$(document).ready(function () {
	...
	$('#search').keydown(async function (e) {
		...
		if (e.keyCode === 13) {
			...
			if (query.startsWith('http://') || query.startsWith('https://') || query.startsWith('file://') || query.startsWith('ftp://') || query.startsWith('steam://') || query.startsWith('magnet:?')) {
				SiteURL = $(this).val()
				console.log("AAA")
			} else {
...
const EXTENSION_AUTH_REQUEST_TIMEOUT = 1200;
...
window.addEventListener('message', function (event) {
	...
	if (event.data.type === 'READY') {
		googleExtensionBridgeDetected = true;
		return;
	}
	if (!['GOOGLE_DRIVE_TOKEN_RESULT', 'SEARCH_DEFAULT_PROVIDER_RESULT'].includes(event.data.type)) return;

	const request = googleExtensionAuthRequests[event.data.requestId];
	if (!request) return;
	delete googleExtensionAuthRequests[event.data.requestId];
	...
	request.resolve(event.data.token || '');
});

function canUseGoogleExtensionAuth() {
	return hasChromeExtensionRuntime() || googleExtensionBridgeDetected || window.self !== window.top;
}

function requestGoogleDriveTokenFromExtension(interactive) {
	if (!canUseGoogleExtensionAuth()) return Promise.reject(new Error('extension_bridge_unavailable'));

	if (hasChromeExtensionRuntime()) {
		return new Promise((resolve, reject) => {
			chrome.runtime.sendMessage({
				source: 'DYG_NEW_TAB_PAGE',
				type: 'GET_GOOGLE_DRIVE_TOKEN',
				interactive: Boolean(interactive)
			}, response => {
				const error = chrome.runtime.lastError;
				if (error || response?.error) {
					let message = error?.message || response.error;
					if (message.includes('bad client id')) {
						message += '. Проверь, что OAuth Client в Google Cloud имеет тип Chrome extension, Item ID равен ' + chrome.runtime.id + ', затем нажми Save и Reload у расширения.';
					}
	...
	return new Promise((resolve, reject) => {
		...
		}, interactive ? 120000 : EXTENSION_AUTH_REQUEST_TIMEOUT);

		googleExtensionAuthRequests[requestId] = { resolve, reject, timeoutId };
		window.postMessage({
			source: 'DYG_NEW_TAB_PAGE',
			type: 'GET_GOOGLE_DRIVE_TOKEN',
			requestId,
...
async function google_login() {
	try {
		await getGoogleDriveToken(true);
		update_google_sync_ui();
...
async function getGoogleDriveToken(interactive = false) {
	...
	if (canUseGoogleExtensionAuth()) {
		try {
			googleDriveAccessToken = await requestGoogleDriveTokenFromExtension(interactive);
			...
			if (!['extension_bridge_timeout', 'extension_bridge_unavailable'].includes(error.message)) {
				throw error;
			}
		}
	}
